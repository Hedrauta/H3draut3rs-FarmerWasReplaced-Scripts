# Todo: rework, keeps breaking, remove set, add "avoid-loops" 
#	depending on length on first rows
...
on_apple = False
# min length avoid: 124  (( math: 2*wosi+2*(wosi-2))) > math for loop : (min_length - 60 - 2*wosi) // 60 
# 
#rework starts here:
while True:
	...
	while not on_apple:
		#debug:
		if applex == 1 and (appley+1)%2 and mx == 1 and (appley-my) <=3:
			pass
		# avoid-loop:
		...
		if (my+1) % 2 and mx == 1 and (appley-2 >= my or appley < my) and my < 30 and not on_apple:
			# quick_print((mx,my),(applex,appley),(appley-2 >= my or appley < my),appley-2 >= my,appley < my)
			move(North)
			mx, my = get_pos_x(), get_pos_y()
			on_apple = applex == mx and appley == my
			yup = move(North)
